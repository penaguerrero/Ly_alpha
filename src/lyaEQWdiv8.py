...
In this version of the code, I attempted to make a plot with the positive EQW values in logarithmic scale, however the result is 
VERY ugly so I decided to keep the old version of the plots and simply add a zoom-in in the stellar part. This is carried out in lyaEQWdiv9.py
...
for foldr in total_chosenfiles:
    ...
    for j in range(0, len(ste_age_and_eqw[1])):
        ...
        if tot > 0.0:
            tot_eqw.append(numpy.log10(tot))   ### logarithmic eqw above 0
        elif tot < 0.0:
            tot_eqw.append(tot)      ### linear eqw
    tot_age_and_eqw = numpy.array([tot_age, tot_eqw])

    # Divide continuous from instantaneous bursts
    logandlinear_ste_eqw = []
    logandlinear_neb_eqw = []
    for i in tot_age:
        if ste_age_and_eqw[1][i] > 0:
            log_ste = numpy.log10(ste_age_and_eqw[1][i])
            logandlinear_ste_eqw.append(log_ste)
        elif ste_age_and_eqw[1][i] < 0:
            logandlinear_ste_eqw.append(ste_age_and_eqw[1][i])
        if neb_age_and_eqw[1][i] > 0:
            log_ste = numpy.log10(neb_age_and_eqw[1][i])
            logandlinear_neb_eqw.append(log_ste)
        elif neb_age_and_eqw[1][i] < 0:
            logandlinear_neb_eqw.append(neb_age_and_eqw[1][i])           
    pair_ste = [tot_age, logandlinear_ste_eqw]
    pair_neb = [tot_age, logandlinear_neb_eqw]
    word = 'fixed'
    if word in foldr:
    ...
    for j in range(0, len(pair_ste[0])):
        temp = [repr(ste_age_and_eqw[0][j]), repr(pair_ste[1][j]), repr(pair_neb[0][j]), repr(pair_neb[1][j]), repr(tot_age_and_eqw[0][j]), repr(tot_age_and_eqw[1][j])]
